...
mod tests {
	...
	fn test_parse_font_name() {
		// Each entry is a string with the following format:
		// "family;postscript_name;expected_family;expected_style;expected_weight;expected_width"
		let tests = vec![
		...
		for test in tests {
			...
			let (family, style, weight, width) =
				parse_font_name(test[0].to_owned(), test[1].to_owned());
			assert_eq!(family, test[2]);
			assert_eq!(style, test[3]);
			assert_eq!(weight.to_string(), test[4]);
			assert_eq!(width, test[5]);
		}
	}
}
